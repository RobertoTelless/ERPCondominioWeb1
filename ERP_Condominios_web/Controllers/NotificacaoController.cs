...
namespace ERP_Condominios_web.Controllers
{
    public class NotificacaoController : Controller
    {
        ...
        public NotificacaoController(INotificacaoAppService baseApps, ILogAppService logApps, IUsuarioAppService usuApps, IUnidadeAppService uniApps)
        {
            baseApp = baseApps;
            logApp = logApps;
            usuApp = usuApps;
        ...
        public ActionResult IncluirNotificacao()
        {
            ...
            // Prepara listas
            List<USUARIO> usuarios = usuApp.GetAllItens(idAss);
            List<USUARIO> moradores = usuarios.Where(p => p.USUA_IN_MORADOR == 1).ToList();
            List<USUARIO> funcs = usuarios.Where(p => p.USUA_IN_FUNCIONARIO == 1).ToList();
            List<UNIDADE> unids = uniApp.GetAllItens(idAss);

            ViewBag.Cats = new SelectList(baseApp.GetAllCategorias(), "CANO_CD_ID", "CANO_NM_NOME");
            ViewBag.Usus = new SelectList(usuarios, "USUA_CD_ID", "USUA_NM_NOME");
            ViewBag.Moradores = new SelectList(moradores, "USUA_CD_ID", "USUA_NM_NOME");
            ViewBag.Funcs = new SelectList(funcs, "USUA_CD_ID", "USUA_NM_NOME");
            ViewBag.Unids = new SelectList(unids, "UNID_CD_ID", "UNID_NM_NOME");

            List<SelectListItem> tipo = new List<SelectListItem>();
            tipo.Add(new SelectListItem() { Text = "Morador", Value = "1" });
            tipo.Add(new SelectListItem() { Text = "Todos Moradores", Value = "2" });
            tipo.Add(new SelectListItem() { Text = "Funcionário", Value = "3" });
            tipo.Add(new SelectListItem() { Text = "Todos Funcionários", Value = "4" });
            tipo.Add(new SelectListItem() { Text = "Unidade", Value = "5" });
            tipo.Add(new SelectListItem() { Text = "Geral", Value = "6" });
        ...
        public ActionResult GerarRelatorioLista()
        {
            ...
            foreach (NOTIFICACAO item in lista)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph(item.NOTI_DT_EMISSAO.ToShortDateString(), meuFont))
                {
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
            ...
            if (filtro != null)
            {
                ...
                if (filtro.NOTI_DT_EMISSAO != null)
                {
                    if (ja == 0)
                    {
                        parametros += "Data: " + filtro.NOTI_DT_EMISSAO.ToShortDateString();
                        ja = 1;
                    }
                    else
                    {
                        parametros += " e Data: " + filtro.NOTI_DT_EMISSAO.ToShortDateString();
                    }
                }
        ...
        public ActionResult GerarRelatorioDetalhe()
        {
            ...
            {
                cell = new PdfPCell(new Paragraph("Emissão: " + aten.NOTI_DT_EMISSAO.ToShortDateString(), meuFont));
                cell.Border = 0;
                cell.Colspan = 1;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
