...
namespace ERP_Condominios_web.Controllers
{
    public class AdministracaoController : Controller
    {
        ...
        public AdministracaoController(IUsuarioAppService baseApps, ILogAppService logApps, ICargoAppService carApps)
        {
            baseApp = baseApps;
            logApp = logApps;
            carApp = carApps;
       ...
       [HttpGet]
        public ActionResult MontarTelaUsuario()
        {
            ...
            ViewBag.Cargos = new SelectList((List<CARGO>)Session["Cargos"], "CARG_CD_ID", "CARG_NM_NOME");
            ...
            ViewBag.Usuarios = ((List<USUARIO>)Session["ListaUsuario"]).Count;
            ViewBag.Perfil = usuario.PERFIL.PERF_SG_SIGLA;
           
            ViewBag.UsuariosBloqueados = listaMaster.Where(p => p.USUA_IN_BLOQUEADO == 1).ToList().Count;
            ViewBag.UsuariosHoje = listaMaster.Where(p => p.USUA_IN_BLOQUEADO == 0 & p.USUA_DT_ACESSO == DateTime.Today.Date).ToList().Count;
        ...
        public ActionResult FiltrarUsuario(USUARIO item)
        {
            ...
            try
            {
                ...
                Int32 volta = baseApp.ExecuteFilter(item.PERF_CD_ID, item.USUA_NM_NOME, item.USUA_NM_LOGIN, item.USUA_NM_EMAIL, item.CARG_CD_ID, idAss, out listaObj);
       ...
       [HttpGet]
        ...
        public ActionResult IncluirUsuario(UsuarioViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (volta == 1)
                    {
                        String erro = ERP_Condominio.ResourceManager.GetString("M0009", CultureInfo.CurrentCulture);
                        ModelState.AddModelError("", erro);
                        return Json(erro);
                    }
                    if (volta == 2)
                    {
                        String erro = ERP_Condominio.ResourceManager.GetString("M0109", CultureInfo.CurrentCulture);
                        ModelState.AddModelError("", erro);
                        return Json(erro);
                    }
                    if (volta == 3)
                    {
                        String erro = ERP_Condominio.ResourceManager.GetString("M0013", CultureInfo.CurrentCulture);
                        ModelState.AddModelError("", erro);
                        return Json(erro);
                    }
                    if (volta == 4 )
                    {
                        String erro = ERP_Condominio.ResourceManager.GetString("M0014", CultureInfo.CurrentCulture);
                        ModelState.AddModelError("", erro);
                        return Json(erro);
                    }
                    ...
                    // Cria pastas
                    String caminho = "/Imagens/" + item.COND_CD_ID.ToString() + "/Usuarios/" + item.USUA_CD_ID.ToString() + "/Fotos/";
                    Directory.CreateDirectory(Server.MapPath(caminho));
                    caminho = "/Imagens/" + item.COND_CD_ID.ToString() + "/Usuarios/" + item.USUA_CD_ID.ToString() + "/Anexos/";
                    Directory.CreateDirectory(Server.MapPath(caminho));

                    // Sucesso
                    listaMaster = new List<USUARIO>();
                    Session["ListaUsuario"] = null;
                    Session["IdVolta"] = item.USUA_CD_ID;
                    return Json(1);
                }
                catch (Exception ex)
                {
        ...
        public ActionResult EditarUsuario(UsuarioViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    Int32? idAss = (Int32)Session["IdAssinante"];
                    USUARIO item = Mapper.Map<UsuarioViewModel, USUARIO>(vm);
                    Int32 volta = baseApp.ValidateEdit(item, (USUARIO)Session["ObjetoAntes"], usuarioLogado, idAss);
                    ...
                    if (volta == 2)
                    {
                        ModelState.AddModelError("", ERP_Condominio.ResourceManager.GetString("M0013", CultureInfo.CurrentCulture));
                        return View(vm);
                    }
                    if (volta == 3)
                    {
                        ModelState.AddModelError("", ERP_Condominio.ResourceManager.GetString("M0014", CultureInfo.CurrentCulture));
                        return View(vm);
                    }
        ...
        public ActionResult BloquearUsuario(Int32 id)
        {
            ...
            // Executar
            USUARIO item = baseApp.GetItemById(id);
            Session["ObjetoAntes"] = (USUARIO)Session["Usuario"];
            item.USUA_IN_BLOQUEADO = 1;
            item.USUA_DT_BLOQUEIO = DateTime.Today;
        ...
        public ActionResult VoltarAnexoUsuario()
        {
            ...
            return RedirectToAction("EditarUsuario", new { id = (Int32)Session["IdVolta"] });
        }
        ...
        public ActionResult UploadFileUsuario(HttpPostedFileBase file)
        {
            ...
            if (file == null)
            {
                ModelState.AddModelError("", ERP_Condominio.ResourceManager.GetString("M0011", CultureInfo.CurrentCulture));
                return RedirectToAction("VoltarAnexoUsuario");
            }

            USUARIO item = baseApp.GetById((Int32)Session["IdVolta"]);
            ...
            {
                ModelState.AddModelError("", ERP_Condominio.ResourceManager.GetString("M0012", CultureInfo.CurrentCulture));
                return RedirectToAction("VoltarAnexoUsuario");
            }

            String caminho = "/Imagens/" + item.COND_CD_ID.ToString() + "/Usuarios/" + item.USUA_CD_ID.ToString() + "/Anexos/";
        ...
        public ActionResult UploadFotoUsuario(HttpPostedFileBase file)
        {
            ...
            {
                ModelState.AddModelError("", ERP_Condominio.ResourceManager.GetString("M0015", CultureInfo.CurrentCulture));
            }
            return RedirectToAction("VoltarAnexoUsuario");
        }
        ...
        public ActionResult GerarRelatorioLista()
        {
            ...
            foreach (USUARIO item in lista)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph(item.PERFIL.PERF_DS_DESCRICAO, meuFont))
                {
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Paragraph(CrossCutting.Formatters.DecimalFormatter(item.USUA_NR_ACESSOS), meuFont))
                {
                    VerticalAlignment = Element.ALIGN_MIDDLE,
                    HorizontalAlignment = Element.ALIGN_LEFT
                };
            ...
            if (filtro != null)
            {
                ...
                if (filtro.PERF_CD_ID > 0)
                {
                    if (ja == 0)
                    {
                        parametros += "Perfil: " + filtro.PERFIL.PERF_DS_DESCRICAO;
                        ja = 1;
                    }
                    else
                    {
                        parametros += " e Perfil: " + filtro.PERFIL.PERF_DS_DESCRICAO;
                    }
                }
                if (ja == 0)
                {
        ...
        public ActionResult GerarRelatorioDetalhe()
        {
            ...
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
            table.AddCell(cell);
            cell = new PdfPCell(new Paragraph("Perfil: " + aten.PERFIL.PERF_DS_DESCRICAO, meuFont));
            cell.Border = 0;
            cell.Colspan = 3;
            cell.VerticalAlignment = Element.ALIGN_MIDDLE;
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
            ...
            if (aten.USUA_DT_BLOQUEIO != null)
            {
                cell = new PdfPCell(new Paragraph("Data Bloqueio: " + aten.USUA_DT_BLOQUEIO.Value.ToShortDateString(), meuFont));
                cell.Border = 0;
                cell.Colspan = 1;
                cell.VerticalAlignment = Element.ALIGN_MIDDLE;
                cell.HorizontalAlignment = Element.ALIGN_LEFT;
