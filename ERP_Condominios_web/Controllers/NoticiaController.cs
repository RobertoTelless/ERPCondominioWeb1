    ...
    public class NoticiaController : Controller
    {
        ...
        public ActionResult FiltrarNoticia(NOTICIA item)
        {
            ...
            try
            {
                ...
                Int32 volta = baseApp.ExecuteFilter(item.NOTC_NM_TITULO, item.NOTC_NM_AUTOR, item.NOTC_DATA_AUTOR, item.NOTC_TX_TEXTO, item.NOTC_LK_LINK, out listaObj);
       ...
       [HttpGet]
        ...
        public ActionResult IncluirNoticia(NoticiaViewModel vm)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    item.NOTC_AQ_ARQUIVO = "~/Imagens/p_big2.jpg";
                    volta = baseApp.ValidateEdit(item, item, usuarioLogado, idAss);
        ...
        public ActionResult UploadFotoNoticia(HttpPostedFileBase file)
        {
            ...
            // Gravar registro
            item.NOTC_AQ_ARQUIVO = "~" + caminho + fileName;
            objetoAntes = item;
            Int32 volta = baseApp.ValidateEdit(item, objetoAntes);
            return RedirectToAction("VoltarAnexoNoticia");
        }
